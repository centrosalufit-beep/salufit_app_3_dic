...
async function checkRateLimit(key, maxAttempts, windowSeconds) {
    ...
    await db.runTransaction(async (t) => {
        ...
        const attempts = (data.attempts || []).filter((ts) => ts > cutoff);
        if (attempts.length >= maxAttempts) {
            throw new functions.https.HttpsError("resource-exhausted", "Demasiados intentos. Espera unos minutos.");
        }
        attempts.push(now);
...
// ═══════════════════════════════════════════════════════════════
exports.consumirTokenPorQR = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Debes iniciar sesión.");
    }
    const scannerUid = context.auth.uid;
    const scannedUid = data?.scannedUserId?.trim() || "";
    ...
    try {
        const result = await db.runTransaction(async (t) => {
            ...
            const tokensRestantes = passSnap.data()?.tokensRestantes ?? 0;
...
// ═══════════════════════════════════════════════════════════════
exports.setStrongPassword = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Debes iniciar sesión.");
    }
    ...
    if (!isStrongPassword(newPassword)) {
        throw new functions.https.HttpsError("invalid-argument", "La contraseña debe tener al menos 12 caracteres, una mayúscula, una minúscula y un número.");
    }
...
exports.deleteUserData = functions.https.onCall(async (data, context) => {
    ...
    const targetUid = data?.targetUserId?.trim() || callerUid;
...
// ═══════════════════════════════════════════════════════════════
exports.cancelarReserva = (0, https_1.onRequest)(async (req, res) => {
