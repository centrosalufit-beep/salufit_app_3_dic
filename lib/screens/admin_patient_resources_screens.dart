...
// ==================================================
// PANTALLA 1: GESTOR DE MATERIAL (EJERCICIOS)
// ==================================================
...
class _AdminPatientMaterialScreenState extends State<AdminPatientMaterialScreen> {
  ...
  Future<void> _borrarEjercicio(String assignmentId) async {
    bool? confirm = await showDialog(context: context, builder: (c) => AlertDialog(title: const Text('Borrar Ejercicio'), content: const Text('¿Eliminar esta pauta?'), actions: [TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('Cancelar')), TextButton(onPressed: () => Navigator.pop(c, true), child: const Text('Borrar', style: TextStyle(color: Colors.red)))]));
    if (confirm != true) return;
    await FirebaseFirestore.instance.collection('exercise_assignments').doc(assignmentId).delete();
  }

  void _abrirAsignador() {
    showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) => Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom), child: _ExerciseSelector(userId: widget.userId, instruccionesDefecto: _instruccionesDefecto)));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text("Gestión Material", style: TextStyle(fontSize: 14)), Text(widget.userName, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))]), backgroundColor: Colors.blue, foregroundColor: Colors.white),
...
// ==================================================
// PANTALLA 2: GESTOR DE DOCUMENTOS
// ==================================================
class AdminPatientDocumentsScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String viewerRole; // Necesario para permitir borrar
...
class _AdminPatientDocumentsScreenState extends State<AdminPatientDocumentsScreen> {
  ...
  Future<void> _borrarDocumentoSeguro(String docId, String urlPdf) async {
    ...
    if (widget.viewerRole != 'admin') { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Solo administradores pueden borrar documentos"))); return; }
    TextEditingController c1 = TextEditingController(); TextEditingController c2 = TextEditingController();
    bool? auth = await showDialog(context: context, builder: (c) => AlertDialog(title: const Text("Borrado Seguro"), content: Column(mainAxisSize: MainAxisSize.min, children: [const Text("Se requiere doble firma admin:"), TextField(controller: c1, obscureText: true, decoration: const InputDecoration(labelText: "Clave 1")), TextField(controller: c2, obscureText: true, decoration: const InputDecoration(labelText: "Clave 2"))]), actions: [ElevatedButton(onPressed: () => Navigator.pop(c, (c1.text == "Clave.2020" && c2.text == "MarciCanela2023*")), child: const Text("BORRAR"))]));
    if (auth != true) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Claves incorrectas"), backgroundColor: Colors.red)); return; }
  ...
  Future<void> _subirPdf() async {
    ...
    if (result != null) {
      ...
      try {
        ...
        String id = '${widget.userId}_${DateTime.now().millisecondsSinceEpoch}';
        await FirebaseFirestore.instance.collection('documents').doc(id).set({'id': id, 'userId': widget.userId, 'titulo': file.name.replaceAll('.pdf', ''), 'tipo': 'Personalizado', 'firmado': false, 'urlPdf': url, 'fechaCreacion': FieldValue.serverTimestamp()});
  ...
  Future<void> _enviarPlantilla(String titulo) async {
    String id = '${widget.userId}_${titulo.replaceAll(' ', '_')}';
    await FirebaseFirestore.instance.collection('documents').doc(id).set({'id': id, 'userId': widget.userId, 'titulo': titulo, 'tipo': 'Legal', 'firmado': false, 'urlPdf': '', 'fechaCreacion': FieldValue.serverTimestamp()});
    if(mounted) { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Plantilla enviada"), backgroundColor: Colors.green)); }
  }
  ...
  void _mostrarOpciones() {
    showModalBottomSheet(context: context, builder: (c) => Container(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(leading: const Icon(Icons.upload_file, color: Colors.blue), title: const Text("Subir PDF Personalizado"), onTap: () { Navigator.pop(c); _subirPdf(); }),
      const Divider(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text("Gestión Documentos", style: TextStyle(fontSize: 14)), Text(widget.userName, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))]), backgroundColor: Colors.orange, foregroundColor: Colors.white),
      floatingActionButton: FloatingActionButton.extended(onPressed: _mostrarOpciones, backgroundColor: Colors.orange, icon: const Icon(Icons.add, color: Colors.white), label: const Text("NUEVO DOCUMENTO", style: TextStyle(color: Colors.white))),
      body: _isUploading ? const Center(child: CircularProgressIndicator()) : StreamBuilder<QuerySnapshot>(
...
// --- WIDGET AUXILIAR: SELECTOR DE EJERCICIOS (Reutilizado) ---
...
class _ExerciseSelectorState extends State<_ExerciseSelector> {
  ...
  Future<void> _asignar(Map<String, dynamic> exerciseData) async {
    String id = '${widget.userId}_${exerciseData['codigoInterno']}_${DateTime.now().millisecondsSinceEpoch}';
    await FirebaseFirestore.instance.collection('exercise_assignments').doc(id).set({'id': id, 'userId': widget.userId, 'exerciseId': exerciseData['codigoInterno'], 'fechaAsignacion': DateTime.now().toIso8601String(), 'instrucciones': _instructionsController.text, 'profesionalId': 'App', 'completado': false, 'feedback': null});
  ...
  @override Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(20), height: 600, child: Column(children: [
      const Text('Seleccionar Ejercicio', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 10),
      ...
      const SizedBox(height: 10), TextField(controller: _instructionsController, maxLines: 2, decoration: const InputDecoration(labelText: 'Instrucciones', border: OutlineInputBorder())),
      const SizedBox(height: 10),
      Expanded(child: StreamBuilder<QuerySnapshot>(stream: FirebaseFirestore.instance.collection('exercises').orderBy('orden').limit(100).snapshots(), builder: (c, s) {
         if(!s.hasData) return const Center(child:CircularProgressIndicator());
         var f = s.data!.docs.where((d) => (d.data() as Map<String,dynamic>)['nombre'].toString().toLowerCase().contains(_search)).toList();
         return ListView.separated(itemCount:f.length, separatorBuilder:(c,i)=>const Divider(), itemBuilder:(c,i) { var dt=f[i].data() as Map<String,dynamic>; return ListTile(title:Text(dt['nombre']), trailing:ElevatedButton(onPressed:()=>_asignar(dt), child:const Text('ASIGNAR'))); });
      }))
    ]));
  }
}